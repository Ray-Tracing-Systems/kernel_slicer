                                             ...
                                             const kslicer::IntersectionShader2& a_foundIS2)
{
  ...
  for(const auto& mainFunc : a_methodsToGenerate)
  {
    ...
    for(size_t i=mainFunc.startDSNumber; i<mainFunc.endDSNumber; i++)
    {
      ...
      {
        if(!internalKernel && !isServeceKernel && !a_classInfo.pShaderCC->IsISPC() && pFoundKernel)
        {
  ...
  data["DescriptorSetsAll"] = std::vector<json>();
    ...
    // check for CommitDeviceData and GetExecutionTime and other specific functions
  {
    ...
    auto pGetTempByfferSize  = a_classInfo.allMemberFunctions.find("GetTempBufferSize");
    if(haveLocalContainers && pGetTempByfferSize == a_classInfo.allMemberFunctions.end())
    {
      std::cout << "  [kslicer]: warning, can't find fuction 'GetTempBufferSize', you should define it: 'virtual size_t GetTempBufferSize() const { return ... ; }'" << std::endl;
      std::cout << "  [kslicer]: it will be used for local vectors of a control function " << std::endl;
      data["DefineGetTempBufferSize"] = true;
    }
