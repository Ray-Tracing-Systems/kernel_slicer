...
def cmake_build(build_dir="build", build_type="Release", return_to_root=True, num_threads=1, clearAll = False, enable_ispc=False, enable_cuda = False):
    if not os.path.isdir(build_dir):
        os.mkdir(build_dir)
    if clearAll:
        for path in os.listdir(build_dir): # clear everything in build folder to be sure we have rebuild sample
            path_v2 = os.path.join(build_dir, path)
