...
{ 
  ...
  inline static unsigned int color_pack_rgba(const vfloat4 rel_col)
  {
    static const vfloat4 const_255 = splat(255.0f);
    const vuint4 rgba = to_uint32(rel_col*const_255);
  ...
  {
    static const vfloat4 const_255 = splat(255.0f);
    const vuint4 rgba = to_uint32(cvex::shuffle_zyxw(rel_col)*const_255);
    return (rgba[3] << 24) | (rgba[2] << 16) | (rgba[1] << 8) | rgba[0];
  }
  ...
  static inline void transpose4(const vfloat4 __restrict in_rows[4], vfloat4 __restrict out_rows[4])
  {
    ...
    cvex::store(rows+0, in_rows[0]);
    cvex::store(rows+4, in_rows[1]);
    cvex::store(rows+8, in_rows[2]);
    cvex::store(rows+12, in_rows[3]);
  
    out_rows[0] = vfloat4{rows[0], rows[4], rows[8],  rows[12]};
    out_rows[1] = vfloat4{rows[1], rows[5], rows[9],  rows[13]};
    out_rows[2] = vfloat4{rows[2], rows[6], rows[10], rows[14]};
    out_rows[3] = vfloat4{rows[3], rows[7], rows[11], rows[15]};
  }
