...
{  
  ...
  static inline bool test_bits_all(const vuint4 v)  { return !test_bits_any(vuint4(vmvnq_u32(v))); }
  static inline bool test_bits_all(const vint4 v)   { return !test_bits_any(vint4(vmvnq_s32(v))); }
  static inline bool test_bits_all(const vfloat4 v) { return test_bits_all(as_int32(v)); }
 
  static inline bool any_of (const vint4 a) { return test_bits_any(a); }
  static inline bool all_of (const vint4 a) { return test_bits_all(a); }
  ...
  static inline float hmax3(const vfloat4 a_val) 
  {
    const float32x2_t xy = vget_low_f32(a_val.data);
    const float32x2_t zw = vget_high_f32(a_val.data);
    const float32x2_t zz = vdup_lane_f32(zw,0);
    const float32x2_t ab = vpmax_f32(xy,zz);
    const float32x2_t bb = vdup_lane_f32(ab,1);
    return vget_lane_f32(vpmax_f32(ab,bb), 0);
  }

  static inline float hmin3(const vfloat4 a_val) 
  {
    const float32x2_t xy = vget_low_f32(a_val.data);
    const float32x2_t zw = vget_high_f32(a_val.data);
    const float32x2_t zz = vdup_lane_f32(zw,0);
    const float32x2_t ab = vpmin_f32(xy,zz);
    const float32x2_t bb = vdup_lane_f32(ab,1);
    return vget_lane_f32(vpmin_f32(ab,bb), 0);
  }

  static inline float hmax(const vfloat4 a_val) 
  {
    const float32x2_t xy = vget_low_f32(a_val.data);
    const float32x2_t zw = vget_high_f32(a_val.data);
    const float32x2_t ab = vpmax_f32(xy,zw);
    const float32x2_t bb = vdup_lane_f32(ab,1);  
    return vget_lane_f32(vpmax_f32(ab,bb), 0);
  }

  static inline float hmin(const vfloat4 a_val) 
  {
    const float32x2_t xy = vget_low_f32(a_val.data);
    const float32x2_t zw = vget_high_f32(a_val.data);
    const float32x2_t ab = vpmin_f32(xy,zw);
    const float32x2_t bb = vdup_lane_f32(ab,1);  
    return vget_lane_f32(vpmin_f32(ab,bb), 0);
  }
  ...
  static inline vfloat4 fabs(const vfloat4 a)
  {
    CVEX_ALIGNED(16) float mres[4];
    vst1q_f32(mres, a);
    const vfloat4 res = {::fabsf(mres[0]), ::fabsf(mres[1]), ::fabsf(mres[2]), ::fabsf(mres[3])};
    return res;
  }
  ...
  inline static unsigned int color_pack_rgba(const vfloat4 rel_col)
  {
    ...
    const vuint4 vrgba      = to_uint32(vfloat4(vmulq_f32(rel_col, const_255)));
  ...
  {
    const vfloat4 const_255 = { 255.0f, 255.0f, 255.0f, 255.0f };
    const vuint4 vrgba      = to_uint32(vfloat4(vmulq_f32(cvex::shuffle_zyxw(rel_col), const_255)));
...
static inline cvex::vuint4 operator> (const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vcgtq_f32(a, b); }
static inline cvex::vuint4 operator< (const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vcltq_f32(a, b); }
static inline cvex::vuint4 operator>=(const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vcgeq_f32(a, b); }
static inline cvex::vuint4 operator<=(const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vcleq_f32(a, b); }
static inline cvex::vuint4 operator==(const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vceqq_f32(a, b); }
static inline cvex::vuint4 operator!=(const cvex::vfloat4 a, const cvex::vfloat4 b) { return (uint32x4_t)vmvnq_u32(vceqq_f32(a, b)); }
