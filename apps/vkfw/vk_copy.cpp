...
vkfw::SimpleCopyHelper::SimpleCopyHelper()
{
...
vkfw::SimpleCopyHelper::SimpleCopyHelper(VkPhysicalDevice a_physicalDevice, VkDevice a_device,
...
void vkfw::SimpleCopyHelper::UpdateBuffer(VkBuffer a_dst, size_t a_dstOffset, const void* a_src, size_t a_size)
{
  ...
  for(size_t currPos = 0; currPos < a_size; currPos += stagingSize) // this is very simple implementation; #TODO: use pin-pong shceme please in advanced class 
  {
...
void vkfw::SimpleCopyHelper::ReadBuffer(VkBuffer a_src, size_t a_srcOffset, void* a_dst, size_t a_size) // #TODO: implement in loop or pin-pong for large buffers !!!
{
  ...
  if (a_size > stagingSize)
  {
    ...
    strOut << "[vk_utils::SimpleCopyHelper::ReadBuffer]: too large input size " << a_size << ", please allocate larger staging buff";
    RUN_TIME_ERROR(strOut.str().c_str());
  }
...
void vkfw::SimpleCopyHelper::UpdateImage(VkImage a_image, const void* a_src, int a_width, int a_height, int a_bpp) // #TODO: implement in loop or pin-pong for large textures
{
...
vkfw::PinPongCopyHelper::PinPongCopyHelper(VkPhysicalDevice a_physicalDevice, VkDevice a_device, VkQueue a_transferQueue, uint32_t a_transferQueueIDX, size_t a_stagingBuffSize) : SimpleCopyHelper()
{
  physDev = a_physicalDevice;
  dev     = a_device;
  queue   = a_transferQueue;
...

  {
    ...
    bufferCreateInfo.usage       = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT; 
    bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // bufferStaging is exclusive to a single queue family at a time
    VK_CHECK_RESULT(vkCreateBuffer(a_device, &bufferCreateInfo, NULL, staging+0)); // create bufferStaging
    VK_CHECK_RESULT(vkCreateBuffer(a_device, &bufferCreateInfo, NULL, staging+1)); // create bufferStaging
    ...
    VkMemoryAllocateInfo allocateInfo = {};
    allocateInfo.sType           = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocateInfo.allocationSize  = memoryRequirements.size*2; // specify required memory.
    allocateInfo.memoryTypeIndex = vk_utils::FindMemoryType(memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, physDev);
    VK_CHECK_RESULT(vkAllocateMemory(a_device, &allocateInfo, NULL, &stagingBuffMemory)); // allocate memory on device.
...
vkfw::PinPongCopyHelper::~PinPongCopyHelper()
{
...
void vkfw::PinPongCopyHelper::SubmitCopy(VkBuffer a_dst, size_t a_dstOffset, size_t a_size, int a_currStagingId)
{
...
void vkfw::PinPongCopyHelper::UpdateBuffer(VkBuffer a_dst, size_t a_dstOffset, const void* a_src, size_t a_size)
{
  assert(a_dstOffset % 4 == 0);
  assert(a_size      % 4 == 0);
  ...
  if (a_size <= 65536)
  {
    ...
    submitInfo.pCommandBuffers    = &cmdBuff; 
    VK_CHECK_RESULT(vkQueueSubmit(queue, 1, &submitInfo, fence));
    VK_CHECK_RESULT(vkWaitForFences(dev, 1, &fence, VK_TRUE, vk_utils::FENCE_TIMEOUT));
  ...
  for(; currPos < a_size; currPos += stagingSizeHalf) // use pin-pong shceme 
  {
    size_t currCopySize = std::min(a_size - currPos, stagingSizeHalf);

    // (0) begin (copy staging[prev] ==> result) in parallell with further vkMapMemory/memcpy/vkUnmapMemory
    //
    if(currPos != 0) 
    ...
    //
    if(currPos != 0) 
      vkWaitForFences(dev, 1, &fence, VK_TRUE, vk_utils::FENCE_TIMEOUT);
...
vkfw::ComputeCopyHelper::ComputeCopyHelper(VkPhysicalDevice a_physicalDevice, VkDevice a_device,
                                                                        ...
                                                                        a_transferQueueIDX, a_stagingBuffSize)
{
  ...
  //
  {
    ...
    allocateInfo.memoryTypeIndex = vk_utils::FindMemoryType(memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, a_physicalDevice); 
  ...
  // (2) we can not write DescriptorSet untill we know the input buffer, but we can make layouts and pool and allocate DS.
  //
  {
  ...
  { 
    ...
    std::vector<uint32_t> code = vk_utils::ReadFile(a_csCopyPath);
    assert(code.size() != 0);
...
vkfw::ComputeCopyHelper::~ComputeCopyHelper()
{
...
void vkfw::ComputeCopyHelper::ReadBuffer(VkBuffer a_src, size_t a_srcOffset, void* a_dst, size_t a_size)
{
