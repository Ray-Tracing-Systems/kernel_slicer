...
{
  auto widthSmall  = w/4;
  auto heightSmall = h/4;

  std::vector<float4> brightPixels(w*h);
  std::vector<float4> downsampledImage(widthSmall*heightSmall);
  std::vector<float4> tempImage(widthSmall*heightSmall);
  ...
  //
  kernel2D_DownSample4x(m_widthSmall, m_heightSmall, brightPixels.data(), 
                        downsampledImage.data());

  // (3) GaussBlur (m_downsampledImage => m_downsampledImage)
  //
  kernel2D_BlurX(m_widthSmall, m_heightSmall, downsampledImage.data(), 
                 tempImage.data()); // m_downsampledImage => m_tempImage

  kernel2D_BlurY(m_widthSmall, m_heightSmall, tempImage.data(), 
                 downsampledImage.data()); // m_tempImage => m_downsampledImage

  // (4) MixAndToneMap(inData4f, m_downsampledImage) => outData1ui
  //
  kernel2D_MixAndToneMap(w,h, inData4f, downsampledImage.data(), 
                         outData1ui);
