...
SceneManager::SceneManager(VkDevice a_device, VkPhysicalDevice a_physDevice, uint32_t a_transferQId, uint32_t a_graphicsQId, 
                           std::shared_ptr<vkfw::ICopyEngine> a_copyHelper, bool useRTX, bool debug) : 
                           m_device(a_device), m_physDevice(a_physDevice), m_transferQId(a_transferQId), m_graphicsQId(a_graphicsQId), m_useRTX(useRTX), m_debug(debug), m_pCopyHelper(a_copyHelper)
{
...
void SceneManager::BuildAllBLAS()
{
  ...
  VkCommandPool commandPool = vk_utils::CreateCommandPool(m_device, m_physDevice, VK_QUEUE_GRAPHICS_BIT,
...
//    for(size_t infoIdx = 0; infoIdx < m_blasOffsetInfo.size(); infoIdx++)
  ...
  vk_utils::ExecuteCommandBuffersNow(allCmdBufs, m_graphicsQ, m_device);
  allCmdBufs.clear();
...
void SceneManager::BuildTLAS()
{
  ...
  for(const auto& inst : m_instanceInfos)
  {
    ...
    instance.instanceCustomIndex = inst.inst_id;
    instance.mask = 0xFF;
  ...
  auto memReqs = vkfw::CreateBuffer(m_device, sizeof(VkAccelerationStructureInstanceKHR) * geometryInstances.size(),
  ...
  vk_rt_utils::RTScratchBuffer scratchBuffer = vk_rt_utils::allocScratchBuffer(m_device, m_physDevice, sizeInfo.buildScratchSize);
  ...
  VkCommandPool commandPool = vk_utils::CreateCommandPool(m_device, m_physDevice, VK_QUEUE_GRAPHICS_BIT,
                                                          VkCommandPoolCreateFlagBits(0));
  VkCommandBuffer commandBuffer = vk_utils::CreateCommandBuffers(m_device, commandPool, 1)[0];
  VkCommandBufferBeginInfo cmdBufInfo{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
