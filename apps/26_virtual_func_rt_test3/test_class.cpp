...
{ 
  ...
  m_materials[0]  = new EmptyMaterial()                                   ;
  m_materials[1]  = new LambertMaterial(float3(0.5,0.5,0.5))              ;
  m_materials[2]  = new LambertMaterial(float3(0.6,0.0235294,0.0235294))  ;
  m_materials[3]  = new LambertMaterial(float3(0.0235294, 0.6, 0.0235294));
  m_materials[4]  = new GGXGlossyMaterial(float3(0.6,0.6,0.1))            ;
  m_materials[5]  = new LambertMaterial(float3(0.08, 0.14,0.26), true)    ;
  m_materials[6]  = new PerfectMirrorMaterial(true)                       ;
  m_materials[7]  = new LambertMaterial(float3(0.25,0.0,0.5))             ;
  m_materials[8]  = new PerfectMirrorMaterial(false)                      ;
  m_materials[9]  = new PerfectMirrorMaterial(false)                      ;
  m_materials[10] = new EmissiveMaterial(20.0f)                           ;
...
void TestClass::kernel_RealColorToUint32(uint tid, uint* mid, uint* out_color)
{
  ...
  if(mid2 != uint(-1)) 
  {
    const float3 color = m_materials[mid2]->GetColor();
...
{
  return m_materials[mid2]->SampleAndEvalBxDF(rayPosAndNear, rayDirAndFar, hit, rands, this);
}
...
void  TestClass::kernel_NextBounce(uint tid, uint* mid, const Lite_Hit* in_hit, const float2* in_bars, 
                                   ...
                                   float4* accumColor, float4* accumThoroughput)
  {
    ...
    const BxDFSample matSam   = m_materials[mid2]->SampleAndEvalBxDF(*rayPosAndNear, *rayDirAndFar, hit, uv, this);
