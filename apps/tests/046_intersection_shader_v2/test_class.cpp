...
void TestClass::InitScene(int numBoxes, int numTris)
{
  ...
  auto geomId3    = m_pRayTraceImpl->AddGeom_AABB(TAG_SPHERES, (const CRT_AABB*)singleSphereBoxes.data(), singleSphereBoxes.size(), BUILD_HIGH, &spherePtr, 1);

  void* spheresPtrArray[] = {&sphere1, &sphere2}; 
  auto geomId4 = m_pRayTraceImpl->AddGeom_AABB(TAG_SPHERES, (const CRT_AABB*)sphereBoxes.data(), sphereBoxes.size(), BUILD_HIGH, spheresPtrArray, 2);
