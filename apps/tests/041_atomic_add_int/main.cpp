...
int main(int argc, const char** argv)
{
  ...
  std::vector<int32_t> outputArray(8);
  ...
  if(onGPU)
  {
    ...
    pImpl = CreateSimpleTest_Generated(ctx, 256*256);
  }
  else
  ...
  pImpl->CalcAndAccum(256*256, outputArray.data(), unsigned(outputArray.size()));
  
  int32_t outputArray2[8] = {};
