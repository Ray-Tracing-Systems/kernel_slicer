                                    ...
                                    const TextGenSettings& a_settings)
{
  ...
  for(const auto decl : usedDecl)
  {
    ...
    if(a_classInfo.pHostCC->HasSpecConstants() && decl.name.find("KSPEC_") != std::string::npos) { // process specialization constants, remove them from normal constants
  ...
  for (const auto& nk : kernels)
  {
    ...
    if(k.hasIntersectionShader2)
    {
      ...
      kernelJson["IS2_FinishName"] = k.intersectionShader2Info.finishName;
      kernelJson["IS2_HasFinish"]  = (k.intersectionShader2Info.finishName != "");
    }
    else
      kernelJson["IS2_HasFinish"] = false;
